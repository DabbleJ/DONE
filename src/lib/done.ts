...
export type Task = { id: string; title: string; note?: string; due?: string; category: string; assignee: string; project?: string; energy: 'quick' | 'focus' | 'errand'; completed: boolean; createdAt: string; snoozed?: boolean };
...
export const starterData: DoneData = {
  ...
  categories: [{ id: 'home', name: 'Home', color: '#DDF0D9' }, { id: 'family', name: 'Family', color: '#F9DED7' }, { id: 'admin', name: 'Life admin', color: '#DFE7F3' }, { id: 'errands', name: 'Errands', color: '#F5E9C8' }],
...
export const priorityEngine = (tasks: Task[]) => tasks.filter(t => !t.completed && !t.snoozed).sort((a,b) => dueScore(b.due) + (b.energy === 'quick' ? 2 : 0) - dueScore(a.due) - (a.energy === 'quick' ? 2 : 0)).slice(0,3);
...
export function parseCapture(input: string): Omit<Task,'id'|'createdAt'|'completed'> { const lower=input.toLowerCase(); const due=lower.includes('tomorrow')?'Tomorrow':lower.includes('today')?'Today':lower.includes('friday')?'Friday':undefined; const assignee=lower.includes('sam')?'sam':'you'; const category=lower.includes('pick up')||lower.includes('buy')?'errands':lower.includes('school')||lower.includes('kids')?'family':lower.includes('book')||lower.includes('email')?'admin':'home'; const clean=input.replace(/\b(today|tomorrow|friday|for sam|ask sam to)\b/gi,'').replace(/\s+/g,' ').trim(); return {title:clean.charAt(0).toUpperCase()+clean.slice(1),due,category,assignee,energy:category==='errands'?'errand':'quick'}; }
