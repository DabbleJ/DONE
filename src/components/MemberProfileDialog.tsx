...
import { Camera, Check, Circle, FolderKanban, Loader2, Upload } from 'lucide-react';
...
export function MemberProfileDialog({ member, onClose }: { member: Member; onClose: () => void }) {
  const { data, updateMember } = useDone();
  ...
  return <Dialog open onOpenChange={open => { if (!open) onClose(); }}><DialogContent className="max-h-[92vh] overflow-y-auto rounded-[2rem] bg-card sm:max-w-2xl"><DialogHeader><DialogTitle className="text-3xl">Family profile</DialogTitle><DialogDescription>Keep their details useful, personal, and unmistakably them.</DialogDescription></DialogHeader>
    ...
    <div className="mt-2 flex gap-3"><Button variant="outline" onClick={onClose} className="h-12 flex-1 rounded-full font-bold">Cancel</Button><Button onClick={save} disabled={!name.trim() || processingImage} className="h-12 flex-1 rounded-full font-bold">Save profile</Button></div>
