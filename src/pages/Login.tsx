...
import { ArrowRight, Check, Gift, PawPrint } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { supabase } from '@/integrations/supabase/client';
import { useSession } from '@/contexts/SessionContext';

export default function Login(){const{startDemo}=useSession();return <main className="min-h-screen bg-background px-5 py-8 lg:grid lg:grid-cols-2 lg:gap-20 lg:px-16">
  ...
  <section className="mx-auto flex w-full max-w-md items-center"><div className="w-full rounded-[2rem] border bg-card p-6 shadow-[0_22px_70px_rgba(47,51,43,.10)] sm:p-9"><h2 className="text-2xl font-semibold">Let’s make life lighter.</h2><p className="mb-6 mt-1 text-muted-foreground">Sign in or create your family space.</p><Auth supabaseClient={supabase} providers={[]} appearance={{theme:ThemeSupa,variables:{default:{colors:{brand:'#39A852',brandAccent:'#2f9045'},radii:{borderRadiusButton:'999px',buttonBorderRadius:'999px',inputBorderRadius:'14px'}}}}} theme="light"/><div className="my-5 flex items-center gap-3 text-xs text-muted-foreground"><span className="h-px flex-1 bg-border"/>or explore first<span className="h-px flex-1 bg-border"/></div><Button onClick={startDemo} variant="outline" className="h-12 w-full rounded-full border-foreground/20 font-bold">Try the family demo <ArrowRight className="ml-2" size={17}/></Button></div></section>
</main>}
